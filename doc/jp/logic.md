...
## 全体フロー
1. 設定読み込み: `repos.json` から owner→repo のリストを生成し、`GHRepository` に変換（repo_token > org_token > tokenless。環境変数未設定でもフォールバックなし）。ブランチ選択は `all_branches`/`branch_regexes`/`scan_default_branch` で制御。
